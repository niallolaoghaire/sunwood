...
class WC_Gateway_Braintree_PayPal extends WC_Gateway_Braintree {
	...
	public function __construct() {
		...
		parent::__construct(
			...
			wc_braintree(),
			array(
				'method_title'       => __( 'Braintree (PayPal)', 'woocommerce-gateway-braintree' ),
				'method_description' => __( 'Allow customers to securely pay using their PayPal account via Braintree.', 'woocommerce-gateway-braintree' ),
				'supports'           => array(
	...
	public function adjust_token_editor_fields() {
		...
		$fields = array(
			...
			'payer_email' => array(
				'label'   => __( 'Email', 'woocommerce-gateway-braintree' ),
				'editable' => false,
			),
		);
	...
	public function tweak_payment_methods_text( $translated_text, $raw_text, $text_domain ) {
		...
		if ( 'woocommerce-plugin-framework' === $text_domain ) {
			...
			if ( 'Use a new bank account' === $raw_text ) {
				...
				$translated_text = __( 'Use a new PayPal account', 'woocommerce-gateway-braintree' );

			} elseif ( 'Bank Accounts' === $raw_text ) {

				$translated_text = __( 'PayPal Accounts', 'woocommerce-gateway-braintree' );
			}
		}
	...
	public function tweak_my_payment_methods_delete_text( $actions, $token ) {
		...
		if ( $token->is_paypal_account() ) {
			$actions['delete']['name'] = __( 'Unlink', 'woocommerce-gateway-braintree' );
		}
	...
	protected function do_paypal_transaction( WC_Order $order ) {
		...
		if ( $response->transaction_approved() ) {
			...
			$message = sprintf(
				__( '%1$s %2$s %3$s Approved', 'woocommerce-gateway-braintree' ),
				$this->get_method_title(),
				$this->is_test_environment() ? __( 'Sandbox', 'woocommerce-gateway-braintree' ) : '',
				$this->perform_credit_card_authorization() ? __( 'Authorization', 'woocommerce-gateway-braintree' ) : __( 'Payment', 'woocommerce-gateway-braintree' )
			);

			// adds the transaction id (if any) to the order note
			if ( $response->get_transaction_id() ) {
				/* translators: Placeholders: %s - transaction ID */
				$message .= ' ' . sprintf( __( '(Transaction ID %s)', 'woocommerce-gateway-braintree' ), $response->get_transaction_id() );
			}
	...
	protected function get_saved_payment_token_order_note( $token ) {
		...
		return sprintf( __( 'PayPal Account Saved: %s', 'woocommerce-gateway-braintree' ), $token->get_payer_email() );
	}
	...
	protected function get_default_title() {
		...
		return __( 'PayPal', 'woocommerce-gateway-braintree' );
	}
	...
	protected function get_default_description() {
		...
		return __( 'Click the PayPal icon below to sign into your PayPal account and pay securely.', 'woocommerce-gateway-braintree' );
	}
